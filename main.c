...
kTask simpleTask(void* args)
{
	while (1) {
		...
		debug_puts(L_INFO, PSTR("Task 1 does stuff\r\n"));
		_delay_ms(100);
...
kTask simpleTask1(void* args)
{
	while (1) {
		...
		//threads_mutexLock(&mutex0);
		debug_puts(L_INFO, PSTR("Task 2 does stuff\r\n"));
		taskmgr_yield(100);
...
kTask simpleTask2(void* args)
{
	while (1) {
		...
		//threads_mutexLock(&mutex0);
		debug_puts(L_INFO, PSTR("Task 3 does stuff\r\n"));
		_delay_ms(100);
...
kTask simpleTask3(void* args)
{
	while (1) {
		...
		//threads_mutexLock(&mutex0);
		debug_puts(L_INFO, PSTR("Task 4 does stuff\r\n"));
		taskmgr_yield(100);
...
void user_postinit()
{
	t1 = taskmgr_createTask(simpleTask, NULL, 150, 1, KTASK_USER, "1task1");
	t2 = taskmgr_createTask(simpleTask1, NULL, 150, 4, KTASK_USER, "2task2");
	t3 = taskmgr_createTask(simpleTask2, NULL, 150, 1, KTASK_USER, "3task3");
	t4 = taskmgr_createTask(simpleTask3, NULL, 150, 4, KTASK_USER, "4task4");
	return;
}
