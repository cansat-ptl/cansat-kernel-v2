...
void platform_prepareStackFrame(kStackPtr_t regionPointer, kStackSize_t stackSize, kTask_t taskPointer, void* args)
{
	if (regionPointer == NULL) {
		...
		regionPointer += stackSize + CFG_KERNEL_STACK_SAFETY_MARGIN + (CFG_KERNEL_STACK_FRAME_REGISTER_OFFSET + CFG_KERNEL_STACK_FRAME_END_OFFSET);
...
void platform_handleStackCorruption()
{
	...
	kStackSize_t stackSize = handle -> stackSize;
	platform_prepareStackFrame(stackPointer, stackSize, kernel_stackCorruptionHook, NULL);
